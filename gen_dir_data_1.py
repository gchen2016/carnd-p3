...
class ImageDataGen(object):
    ...
    def __init__(self,
        ...
        print("Fisrt 5 training images:")
        [print(i) for i in self.x_train_names[:5]]
        print("Fisrt 5 validation images:")
        [print(i) for i in self.x_valid_names[:5]]
        self.check_and_shuffle_train_index()
    ...
    def check_and_shuffle_train_index(self):
        """
        Shuffling only training index.
        Not shuffling validation for a time slice/sequence validation like in the simulator
        :return:
        """
        if self.shuffle:
            print("\nShuffling training index ...")
            # shuffle list of names + angles
            l = list(zip(self.x_train_names, self.y_train_angles))
            random.shuffle(l)
            self.x_train_names, self.y_train_angles = zip(*l)
            print("\nFisrt 5 training images:")
            [print(i) for i in self.x_train_names[:5]]
            print("\nFisrt 5 validation images:")
            [print(i) for i in self.x_valid_names[:5]]
